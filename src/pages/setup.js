...
export default function Setup() {
  ...
  const { changeState, axios, userType, nickname, venmoId, roomId } = useContext(GlobalContext);
  ...
  const processPicture = async (e) => {
    setLoading(true)
    if (e.target.files?.[0]) {
      let receipt = (await axios.post('/api/processReceipt', {
        body: e.target.files?.[0]
      }, {
        headers: { 'content-type': 'multipart/form-data' },
      })).data;
      setLoading(false);
      let res = await axios.post('/api/createRoom', {
        receipt,
        venmoId
      });
        ...
        { userType === 'payer' ? (
          <>
            ...
            <input
              ...
              value={venmoId}
              onChange={e => {
                let venmoId = e.target.value.slice(0, 35)
                changeState({ venmoId })
              }}
            />
            <br/>
        ...
        <div style={{
          ...
          >
            { loading ? (
              ...
              <>{ userType === 'payer' ? 'Take Picture Of Receipt' : 'Next' }</>
            )}
          </button>
        </div>
        <input
